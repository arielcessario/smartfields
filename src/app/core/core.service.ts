import { LoginStatus } from '../model/auth';
...
export class CoreService {
  ...
  public getCategorias: Subject<any> = new Subject<any>();
  public getDeseos: Subject<any> = new Subject<any>();
  public getLoginStatus: Subject<boolean> = new Subject<boolean>();
  public showToast: Subject<any> = new Subject<any>();
  public onShowSuccess: Subject<any> = new Subject<any>();
  public onShowError: Subject<any> = new Subject<any>();
  ...
  public setLoginStatus(loginStatus: LoginStatus) {
    this.getLoginStatus.next(loginStatus.isLogged);
  }
  ...
  public getEstablecimientoSelected() {
    return localStorage.getItem('establecimientoId')
  ...
  public returnErrors(errors: any) {
    ...
    let response = '';
    for (const x of Object.getOwnPropertyNames(errors)) {
      response = errors[x] + ', ' + response;
    }
    return response;
  }
  ...
  onValueChanged(
    ...
    const fields = Object.getOwnPropertyNames(formErrors);
    // const form = form;
    for (let field = 0; field < fields.length; field++) {
      // clear previous error message (if any)
      formErrors[fields[field]] = '';
      const control = form.controls[fields[field]];
      if (control && control.dirty && !control.valid) {
        const messages = validationMessages[fields[field]];

        const errorsKeys = Object.getOwnPropertyNames(control.errors);
        for (let key = 0; key < errorsKeys.length; key++) {
          formErrors[fields[field]] += messages[errorsKeys[key]] + ' ';
        }
      }
    }
  ...
  public filter(params, values, exact_match, data) {
    ...
    const valores = values.split(',');
    const exactos = exact_match.split(',');

    const respuesta = [];
    for (let y = 0; y < data.length; y++) {
      const columns = Object.keys(data[y]);

      for (let i = 0; i < columns.length; i++) {
        for (let x = 0; x < parametros.length; x++) {
          if (columns[i] === parametros[x]) {
            const base = '' + data[y][Object.keys(data[y])[i]];
            const valor =
              valores.length === 1 ? '' + valores[0] : '' + valores[x];
            let exacto = exactos.length === 1 ? exactos[0] : exactos[x];
            exacto = exacto === 'true';
            const negado = valor.indexOf('!') > -1;
            ...
            if (negado) {
              if (
                (exacto &&
                  this.removeDiacritics(base).toUpperCase() !==
                    this.removeDiacritics(valor)
                      .toUpperCase()
                      .replace('!', '')) ||
                (!exacto &&
                  !this.compareStringSearch(base.replace('!', ''), valor))
              ...
              if (
                (exacto &&
                  this.removeDiacritics(base).toUpperCase() ===
                    this.removeDiacritics(valor).toUpperCase()) ||
                (!exacto && this.compareStringSearch(base, valor))
              ) {
                respuesta.push(data[y]);
                x = parametros.length;
                i = columns.length;
  ...
  compareStringSearch(base, valor) {
    const valores = valor.split(' ');
    let encontrados = 0;

    for (let x = 0; x < valores.length; x++) {
      const val = valores.length > 1 ? valores[x] : valor;

      if (
        this.removeDiacritics(base)
          .toUpperCase()
          .indexOf(this.removeDiacritics(val).toUpperCase()) > -1
      ) {
